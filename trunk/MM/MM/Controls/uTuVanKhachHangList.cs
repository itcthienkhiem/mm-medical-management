...
{
    public partial class uTuVanKhachHangList : uBase
    {
        #region Members
        ...
        {
            Result result = TuVanKhachHangBus.GetTuVanKhachHangList(_fromDate, _toDate, _tenBenhNhan, _tenNguoiTao, _bacSiPhuTrach, _inOut);
            if (result.IsOK)
            {
            ...
            {
                MsgBox.Show(Application.ProductName, result.GetErrorAsString("TuVanKhachHangBus.GetTuVanKhachHangList"), IconType.Error);
                Utility.WriteToTraceLog(result.GetErrorAsString("TuVanKhachHangBus.GetTuVanKhachHangList"));
            }
        }
        ...
        private void OnAdd()
        {
            dlgAddTuVanKhachHang dlg = new dlgAddTuVanKhachHang();
        ...
        private void OnEdit()
        {
            ...
            {
                MsgBox.Show(Application.ProductName, "Vui lòng chọn 1 tư vấn khách hàng.", IconType.Information);
                return;
            }

            DataRow drYKienKhachHang = (dgYKienKhachHang.SelectedRows[0].DataBoundItem as DataRowView).Row;
            dlgAddTuVanKhachHang dlg = new dlgAddTuVanKhachHang(drYKienKhachHang);
            if (dlg.ShowDialog(this) == DialogResult.OK)
            {
                DisplayAsThread();
            }
        ...
        private void OnDelete()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                if (Boolean.Parse(row["Checked"].ToString()))
                {
                    ...
                    {
                        MsgBox.Show(Application.ProductName, "Bạn không thể xóa tư vấn khách hàng của người khác. Vui lòng kiểm tra lại.", IconType.Information);
                        return;
                    }

                    deletedSpecList.Add(row["TuVanKhachHangGUID"].ToString());
                    deletedRows.Add(row);
                }
            }
            ...
            {
                if (MsgBox.Question(Application.ProductName, "Bạn có muốn xóa những tư vấn khách hàng mà bạn đã đánh dấu ?") == DialogResult.Yes)
                {
                    Result result = TuVanKhachHangBus.DeleteTuVanKhachHang(deletedSpecList);
                    if (result.IsOK)
                    {
                    ...
                    {
                        MsgBox.Show(Application.ProductName, result.GetErrorAsString("TuVanKhachHangBus.DeleteTuVanKhachHang"), IconType.Error);
                        Utility.WriteToTraceLog(result.GetErrorAsString("TuVanKhachHangBus.DeleteTuVanKhachHang"));
                    }
                }
            }
            else
                MsgBox.Show(Application.ProductName, "Vui lòng đánh dấu những tư vấn khách hàng cần xóa.", IconType.Information);
        }
        ...
        private void OnPrint(bool isPreview)
        {
            ...
            {
                string exportFileName = string.Format("{0}\\Temp\\TuVanKhachHang.xls", Application.StartupPath);
                if (isPreview)
                {
                    if (!ExportExcel.ExportTuVanKhachHangToExcel(exportFileName, checkedRows))
                        return;
                    else
                    {
                        try
                        {
                            ExcelPrintPreview.PrintPreview(exportFileName, Global.PageSetupConfig.GetPageSetup(Const.TuVanKhachHangTemplate));
                        }
                        catch (Exception ex)
                        {
                ...
                else
                {
                    if (_printDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        else
                        {
                            try
                            {
                                ExcelPrintPreview.Print(exportFileName, _printDialog.PrinterSettings.PrinterName, Global.PageSetupConfig.GetPageSetup(Const.TuVanKhachHangTemplate));
                            }
                            catch (Exception ex)
                            {
                                MsgBox.Show(Application.ProductName, "Vui lòng kiểm tra lại máy in.", IconType.Error);
                ...
                MsgBox.Show(Application.ProductName, "Vui lòng đánh dấu những tư vấn khách hàng cần in.", IconType.Information);
        }
        ...
        private void OnExportToExcel()
        {
            ...
            if (checkedRows.Count > 0)
            {
                ...
                {
                    if (!ExportExcel.ExportTuVanKhachHangToExcel(dlg.FileName, checkedRows))
                        return;
                }
            }
            else
                MsgBox.Show(Application.ProductName, "Vui lòng đánh dấu những tư vấn khách hàng cần xuất excel.", IconType.Information);
        }
        #endregion
        ...
        private void dgYKienKhachHang_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (dgYKienKhachHang.CurrentCell.ColumnIndex == 6)
            {
                ...
                if (row == null) return;
                string yKienKhachHangGUID = row["TuVanKhachHangGUID"].ToString();
                ...
                if (dlg.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    Result result = TuVanKhachHangBus.UpdateKetLuan(yKienKhachHangGUID, ketLuan);
                    if (result.IsOK)
                    {
                    ...
                    {
                        MsgBox.Show(Application.ProductName, result.GetErrorAsString("TuVanKhachHangBus.UpdateKetLuan"), IconType.Error);
                        Utility.WriteToTraceLog(result.GetErrorAsString("TuVanKhachHangBus.UpdateKetLuan"));
                    }
                }
            }
            else OnEdit();
