...
namespace MM.Controls
{
    ...
    public class TNumEditDataGridViewColumn : DataGridViewTextBoxColumn
    {
        ...
        [
            Category("Appearance"),
            DefaultValue(0),
            Description("Min of cell value.")
        ]
        public int Min
        {
            get { return this.NumEditCellTemplate.Min; }
            set
            {
                this.NumEditCellTemplate.Min = value;
                if (this.DataGridView != null)
                {
                    // Update all the existing DataGridViewNumericUpDownCell cells in the column accordingly.
                    DataGridViewRowCollection dataGridViewRows = this.DataGridView.Rows;
                    ...
                    for (int rowIndex = 0; rowIndex < rowCount; rowIndex++)
                    {
                        ...
                        if (dataGridViewCell != null)
                        {
                            // Call the internal SetDecimalPlaces method instead of the property to avoid invalidation 
                            // of each cell. The whole column is invalidated later in a single operation for better performance.
                            dataGridViewCell.SetMin(rowIndex, value);
                        }
                    }
                    this.DataGridView.InvalidateColumn(this.Index);
                    // TODO: Call the grid's autosizing methods to autosize the column, rows, column headers / row headers as needed.
        ...
        [
           ...
           Description("Max of cell value.")
       ]
        public int Max
        {
            get { return this.NumEditCellTemplate.Max; }
            set
            {
                this.NumEditCellTemplate.Max = value;
                if (this.DataGridView != null)
                {
                    // Update all the existing DataGridViewNumericUpDownCell cells in the column accordingly.
                    DataGridViewRowCollection dataGridViewRows = this.DataGridView.Rows;
                    ...
                    for (int rowIndex = 0; rowIndex < rowCount; rowIndex++)
                    {
                        ...
                        if (dataGridViewCell != null)
                        {
                            // Call the internal SetDecimalPlaces method instead of the property to avoid invalidation 
                            // of each cell. The whole column is invalidated later in a single operation for better performance.
                            dataGridViewCell.SetMax(rowIndex, value);
                        }
                    }
                    this.DataGridView.InvalidateColumn(this.Index);
                    // TODO: Call the grid's autosizing methods to autosize the column, rows, column headers / row headers as needed.
    ...
    public class TNumEditDataGridViewCell : DataGridViewTextBoxCell
    {
        ...
        internal void SetMin(int rowIndex, int value)
        {
            m_Min = value;
            if (OwnsEditingControl(rowIndex))
                this.EditingTNumEditBox.Min = value;
        }

        internal void SetMax(int rowIndex, int value)
        {
            m_Max = value;
            if (OwnsEditingControl(rowIndex))
                this.EditingTNumEditBox.Max = value;
        }
