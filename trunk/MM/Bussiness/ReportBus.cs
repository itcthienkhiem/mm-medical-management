...
namespace MM.Bussiness
{
    ...
    {
        private static ChiDinh GetChiDinh(string serviceHistoryGUID, MMOverride db)
        {
            try
            {
                ...
                if (dvcd != null)
                {
                    ...
                    if (ctcd != null)
                        return db.ChiDinhs.SingleOrDefault(c => c.ChiDinhGUID == ctcd.ChiDinhGUID && c.Status == (byte)Status.Actived);
                }
            }
        ...
        public static Result ThongKePhieuThuDichVuVaThuoc(DateTime tuNgay, DateTime denNgay, int type)
        {
            ...
            try
            {
                ...
                string query = string.Format("SELECT PT.ReceiptCode AS SoPhieuThu, PT.ReceiptDate AS NgayPhieuThu, PT.FullName AS TenKhachHang, PT.Address AS DiaChi, CT.[Name] AS TenHangHoa, N'Lần' AS DonViTinh, CT.SoLuong, CT.Price AS DonGia, CT.Discount AS Giam, CAST(((CT.Price - (CT.Price * CT.Discount)/100) * CT.SoLuong) AS float) AS ThanhTien, CASE HinhThucThanhToan WHEN 0 THEN 'TM' WHEN 1 THEN 'CK' WHEN 2 THEN 'TM/CK' WHEN 3 THEN 'BH' WHEN 4 THEN 'CT' END AS HinhThucThanhToan, PT.FileNum AS MaKhachHang, CT.Code AS MaHangHoa, PT.Notes AS GhiChu FROM ReceiptView PT, ReceiptDetailView CT WHERE PT.ReceiptGUID = CT.ReceiptGUID AND PT.Status = 0 AND CT.ReceiptDetailStatus = 0 AND PT.ReceiptDate BETWEEN '{0}' AND '{1}' AND {2} UNION SELECT PT.MaPhieuThuThuoc AS SoPhieuThu, PT.NgayThu AS NgayPhieuThu, PT.TenBenhNhan AS TenKhachHang, PT.DiaChi, CT.TenThuoc AS TenHangHoa, CT.DonViTinh, CT.SoLuong, CT.DonGia, CT.Giam, CAST(((CT.DonGia - (CT.DonGia * CT.Giam)/100) * CT.SoLuong) AS float) AS ThanhTien, CASE HinhThucThanhToan WHEN 0 THEN 'TM' WHEN 1 THEN 'CK' WHEN 2 THEN 'TM/CK' WHEN 3 THEN 'BH' WHEN 4 THEN 'CT' END AS HinhThucThanhToan, PT.MaBenhNhan AS MaKhachHang, CT.MaThuoc AS MaHangHoa, PT.Notes AS GhiChu FROM PhieuThuThuoc PT, ChiTietPhieuThuThuocView CT WHERE PT.PhieuThuThuocGUID = CT.PhieuThuThuocGUID AND PT.Status = 0 AND CT.CTPTTStatus = 0 AND PT.NgayThu BETWEEN '{0}' AND '{1}' AND {2} ORDER BY SoPhieuThu",
                    tuNgay.ToString("yyyy-MM-dd 00:00:00"), denNgay.ToString("yyyy-MM-dd 23:59:59"), subQuery);
                return ExcuteQuery(query);
            }
            catch (System.Data.SqlClient.SqlException se)
