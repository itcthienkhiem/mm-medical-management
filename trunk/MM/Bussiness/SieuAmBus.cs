    ...
    public class SieuAmBus : BusBase
    {
        ...
        public static Result GetKetQuaSieuAmList(string patientGUID, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                ...
                query = string.Format("SELECT  CAST(0 AS Bit) AS Checked, * FROM KetQuaSieuAmView WITH(NOLOCK) WHERE PatientGUID = '{0}' AND NgaySieuAm BETWEEN '{1}' AND '{2}' AND Status = {3} AND LoaiSieuAmStatus = {3} AND PatientArchived = 'False' ORDER BY NgaySieuAm DESC",
                        patientGUID, fromDate.ToString("yyyy-MM-dd HH:mm:ss"), toDate.ToString("yyyy-MM-dd HH:mm:ss"), (byte)Status.Actived);
        ...
        public static Result GetKetQuaSieuAmList2(string patientGUID, DateTime fromDate, DateTime toDate)
        {
            Result result = null;

            try
            {
                string query = string.Format("SELECT * FROM KetQuaSieuAmView WITH(NOLOCK) WHERE PatientGUID = '{0}' AND NgaySieuAm BETWEEN '{1}' AND '{2}' AND Status = {3} AND LoaiSieuAmStatus = {3}",
                        patientGUID, fromDate.ToString("yyyy-MM-dd HH:mm:ss"), toDate.ToString("yyyy-MM-dd HH:mm:ss"), (byte)Status.Actived);
        ...
        public static Result ChuyenBenhAn(string patientGUID, List<DataRow> rows)
        {
            ...
            try
            {
                ...
                using (TransactionScope t = new TransactionScope(TransactionScopeOption.RequiresNew))
                {
                    ...
                    {
                        string ketQuaSieuAmGUID = row["KetQuaSieuAmGUID"].ToString();
                        KetQuaSieuAm ketQuaSieuAm = (from s in db.KetQuaSieuAms
                                                     where s.KetQuaSieuAmGUID.ToString() == ketQuaSieuAmGUID
                                                     select s).FirstOrDefault();

                        if (ketQuaSieuAm == null) continue;

                        //Tracking
                        string desc = string.Format("- KetQuaSieuAmGUID: '{0}': PatientGUID: '{1}' ==> '{2}' (KetQuaSieuAm)",
                            ketQuaSieuAmGUID, ketQuaSieuAm.PatientGUID.ToString(), patientGUID);

                        ketQuaSieuAm.PatientGUID = Guid.Parse(patientGUID);
