            ...
            try
            {
                ...
                else
                {
                    if (tenBenhNhan.Trim() == string.Empty)
                        query = string.Format("SELECT CAST(0 AS Bit) AS Checked, * FROM KetQuaXetNghiem_CellDyn3200View WHERE Status={0} ORDER BY NgayXN DESC",
                            (byte)Status.Actived, tenBenhNhan);
                    else
                        query = string.Format("SELECT CAST(0 AS Bit) AS Checked, * FROM KetQuaXetNghiem_CellDyn3200View WHERE Status={0} AND FullName LIKE N'%{1}%' ORDER BY NgayXN DESC",
                            (byte)Status.Actived, tenBenhNhan);
                }
