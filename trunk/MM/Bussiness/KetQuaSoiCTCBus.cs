        ...
        public static Result GetKetQuaSoiCTCList(string patientGUID, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                ...
                if (Global.StaffType != StaffType.BacSi && Global.StaffType != StaffType.BacSiSieuAm &&
                    ...
                    query = string.Format("SELECT  CAST(0 AS Bit) AS Checked, * FROM KetQuaSoiCTCView WHERE PatientGUID = '{0}' AND NgayKham BETWEEN '{1}' AND '{2}' AND Status = {3} AND Archived = 'False' ORDER BY NgayKham DESC",
                        patientGUID, fromDate.ToString("yyyy-MM-dd HH:mm:ss"), toDate.ToString("yyyy-MM-dd HH:mm:ss"), (byte)Status.Actived);
                else
                    query = string.Format("SELECT  CAST(0 AS Bit) AS Checked, * FROM KetQuaSoiCTCView WHERE PatientGUID = '{0}' AND NgayKham BETWEEN '{1}' AND '{2}' AND Status = {3} AND Archived = 'False' AND BacSiSoi = '{4}' ORDER BY NgayKham DESC",
