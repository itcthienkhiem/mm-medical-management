        ...
        public static Result GetTongTien(bool isFromDateToDate, DateTime fromDate, DateTime toDate, string tenBenhNhan, int type, int type2)
        {
            ...
            try
            {
                ...
                if (isFromDateToDate)
                {
                    if (type == 0) //Tất cả
                    {
                        query = string.Format(@"SELECT SUM(ThanhTien) AS TongTien FROM PhieuThuCapCuuView P WITH(NOLOCK), ChiTietPhieuThuCapCuu C WITH(NOLOCK) 
                                                WHERE P.PhieuThuCapCuuGUID = C.PhieuThuCapCuuGUID AND C.Status = 0 AND NgayThu BETWEEN '{0}' AND '{1}'{2}",
                           fromDate.ToString("yyyy-MM-dd HH:ss:mm"), toDate.ToString("yyyy-MM-dd HH:ss:mm"), subQuery);
                    }
                    else if (type == 1) //Chưa xóa
                    {
                        query = string.Format(@"SELECT SUM(ThanhTien) AS TongTien FROM PhieuThuCapCuuView P WITH(NOLOCK), ChiTietPhieuThuCapCuu C WITH(NOLOCK) 
                                                WHERE P.PhieuThuCapCuuGUID = C.PhieuThuCapCuuGUID AND C.Status = 0 AND P.Status={0} AND NgayThu BETWEEN '{1}' AND '{2}'{3}",
                        (byte)Status.Actived, fromDate.ToString("yyyy-MM-dd HH:ss:mm"), toDate.ToString("yyyy-MM-dd HH:ss:mm"), subQuery);
                    }
                    else //Đã xóa
                    {
                ...
                else
                {
                    ...
                    {
                        query = string.Format(@"SELECT SUM(ThanhTien) AS TongTien FROM PhieuThuCapCuuView P WITH(NOLOCK), ChiTietPhieuThuCapCuu C WITH(NOLOCK) 
                                                WHERE P.PhieuThuCapCuuGUID = C.PhieuThuCapCuuGUID AND C.Status = 0 AND TenBenhNhan LIKE N'%{0}%'{1}", tenBenhNhan, subQuery);
                    }
                    else if (type == 1) //Chưa xóa
                    {
                        query = string.Format(@"SELECT SUM(ThanhTien) AS TongTien FROM PhieuThuCapCuuView P WITH(NOLOCK), ChiTietPhieuThuCapCuu C WITH(NOLOCK) 
                                                WHERE P.PhieuThuCapCuuGUID = C.PhieuThuCapCuuGUID AND C.Status = 0 AND P.Status={0} AND TenBenhNhan LIKE N'%{1}%'{2}",
                        (byte)Status.Actived, tenBenhNhan, subQuery);
                    }
                    else //Đã xóa
                    {
