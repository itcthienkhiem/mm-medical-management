...
namespace MM.Bussiness
{
    public class HoaDonHopDongBus : BusBase
    {
        ...
        public static Result GetHoaDonHopDongList(DateTime fromDate, DateTime toDate, string tenKhachHang, string tenDonVi, string maSoThue, int type)
        {
            Result result = new Result();
            ...
            try
            {
                ...
                string query = string.Format("SELECT CAST(0 AS Bit) AS Checked, *, CASE ChuaThuTien WHEN 'True' THEN 'False' ELSE 'True' END AS DaThuTien FROM HoaDonHopDongView WITH(NOLOCK) WHERE NgayXuatHoaDon BETWEEN '{0}' AND '{1}' AND TenNguoiMuaHang LIKE N'%{2}%' AND TenDonVi LIKE N'%{3}%' AND MaSoThue LIKE N'%{4}%' AND {5} ORDER BY NgayXuatHoaDon DESC",
                           fromDate.ToString("yyyy-MM-dd HH:ss:mm"), toDate.ToString("yyyy-MM-dd HH:ss:mm"), tenKhachHang, tenDonVi, maSoThue, subQuery);
