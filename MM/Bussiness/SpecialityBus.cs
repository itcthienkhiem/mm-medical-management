...
namespace MM.Bussiness
{
    public class SpecialityBus : BusBase
    {
        public static Result GetSpecialityList()
        {
            ...
            try
            {
                string query = string.Format("SELECT CAST(0 AS Bit) AS Checked, * FROM Speciality WITH(NOLOCK) WHERE Status={0} ORDER BY Name", (byte)Status.Actived);
        ...
        public static Result GetSpecialityCount()
        {
            ...
            try
            {
                string query = "SELECT Count(*) FROM Speciality WITH(NOLOCK)";
