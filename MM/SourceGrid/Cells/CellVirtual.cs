...
namespace SourceGrid2.Cells.Virtual
{
	...
	public abstract class CellVirtual : ICellVirtual
	{
		...
		public virtual void StartEdit(Position p_Position, object p_NewStartEditValue)
		{
			if (m_Grid!=null && 
				m_CellModel != null && 
				IsEditing(p_Position) == false &&  //se la cella non � gi� in stato edit
		...
		public virtual void Invalidate(Position p_Position)
		{
			if (m_Grid != null)
			{
				//Non avendo la posizione della cella non conosco quale area invalidare. Ma visto che questa cella pu� essere usata per pi� posizioni credo sia giusto.
