...
namespace MM.Dialogs
{
    public partial class dlgAddPhieuThuCapCuu : dlgBase
    {
        ...
        private string GetGenerateCode()
        {
            ...
            {
                int count = Convert.ToInt32(result.QueryResult);
                return Utility.GetCode("PTCC", count + 1, 7);
            }
            else
            {
                MsgBox.Show(this.Text, result.GetErrorAsString("PhieuThuCapCuuBus.GetPhieuThuCapCuuCount"), IconType.Error);
        ...
        private void OnSaveInfo()
        {
            try
            {
                MethodInvoker method = delegate
                {
                    ...
                    _phieuThuCapCuu.HinhThucThanhToan = (byte)cboHinhThucThanhToan.SelectedIndex;
                    ...
                    for (int i = 0; i < dgChiTiet.RowCount; i++)
                    {
                        ...
                        double tienGiam = Math.Round(((double)ctptcc.SoLuong * (double)ctptcc.DonGia * (double)ctptcc.Giam) / (double)100);
                        double thanhTien = (double)ctptcc.SoLuong * (double)ctptcc.DonGia - tienGiam;

                        ctptcc.ThanhTien = thanhTien;//Convert.ToDouble(row.Cells["ThanhTien"].Value);
                    ...
                    _phieuThuCapCuu.MaPhieuThuCapCuu = maPhieuThu;
